...
function JobSeekers() {
  ...
  const [singleFreelancer, setSingleFreelancer] = React.useState<any>({});
  const [isLoading, setIsLoading] = React.useState<boolean>(false);
  const [totalFreelancers, setTotalFreelancers] = React.useState<number>(0);
  ...
  const getFreelancersHandler = async () => {
    setIsLoading(true);
    const response = await (FreelancerStore.getState() as any).getFreelancers(
      selectedOption.value,
      isChecked,
      1,
      5
    );
    setFreelancers(response.users);
    setTotalFreelancers(response.totalUsers);
    setIsLoading(false);
          ...
          <div className="flex  flex-col gap-y-2">
            <div className="flex items-center justify-between">
              <span className="font-poppins font-bold">
                {selectLoading ? "Loading ..." : selectedOption.label}{" "}
                Freelancers -
                <span className="text-[1rem] text-orange">
                  {" "}
                  {isLoading ? "Loading..." : ` (${totalFreelancers})`}
                </span>
              </span>
                ...
                <tbody>
                  {freelancers?.map((freelancer) => (
                    <tr
                      ...
                      key={freelancer._id}
                    >
                      {freelancer?.isDocumentVerified === "Pending" && (
                        <th
                          scope="row"
                      ...
                      {freelancer?.isDocumentVerified === "is_not_verified" && (
                        <th
                          scope="row"
                          className="px-6 py-4  font-medium text-gray-900 whitespace-nowrap"
                        >
                      ...
                      {freelancer?.isDocumentVerified === "verified" && (
                        <th
                          scope="row"
                          className="px-6 py-4  font-medium text-gray-900 whitespace-nowrap"
                        >
                      ...
                      <td className="px-6 py-4">{freelancer?.email}</td>
                      <td className="px-6 py-4">
                        {freelancer?.phoneNumber || "Not verified"}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <span
                          onClick={() => {
                            ...
                            setSingleFreelancer(freelancer);
                          }}
          ...
          <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none shadow-2xl">
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              <UserModal
                ...
                functionHandler={getFreelancersHandler}
              />
            </div>
          </div>
        )}
